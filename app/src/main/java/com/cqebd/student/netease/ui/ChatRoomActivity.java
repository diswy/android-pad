...
import com.cqebd.student.netease.base.TActivity;
...
//                    }
            ...
            getHandler().postDelayed(() -> initChatRoomFragment(roomName), 50);
        }
    }
...
//        boolean isOpen = Preferences.getRTSRecord();
    ...
    private void endSession() {
        RTSManager2.getInstance().leaveSession(sessionName, new RTSCallback<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                Logger.i("leave session success");
            }

            @Override
            public void onFailed(int code) {
                Logger.i("leave session failed, code:" + code);
            }
