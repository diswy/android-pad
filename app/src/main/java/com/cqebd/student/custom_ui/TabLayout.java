...
package com.cqebd.student.custom_ui;
...
public class TabLayout extends HorizontalScrollView {
    ...
    @NonNull
    public Tab newTab(int dpWidth) {
        Tab tab = sTabPool.acquire();
        if (tab == null) {
            tab = new Tab();
        }
        tab.mParent = this;
        tab.mView = createTabView(tab,dpWidth);
        return tab;
    }
    ...
    private TabView createTabView(@NonNull final Tab tab,int dp) {
        TabView tabView = mTabViewPool != null ? mTabViewPool.acquire() : null;
        if (tabView == null) {
            tabView = new TabView(getContext());
        }
        tabView.setTab(tab);
        tabView.setFocusable(true);
        tabView.setMinimumWidth(dpToPx(dp));
        return tabView;
    }
    ...
    class TabView extends LinearLayout {
        ...
        public TabView(Context context,float sp) {
            super(context);
            if (mTabBackgroundResId != 0) {
                ViewCompat.setBackground(
                        this, AppCompatResources.getDrawable(context, mTabBackgroundResId));
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        private int mSelectedIndicatorWidth =  dpToPx(30);
