...
public class MaterialHeader extends View implements NiftyRefreshLayout.NiftyUIHandler {

    private MaterialProgressDrawable mDrawable;
    private float mScale = 1f;
    private NiftyRefreshLayout refreshLayout;
    ...
    public void setNiftyRefreshLayout(NiftyRefreshLayout layout) {

        final NiftyRefreshLayout.NiftyUIHandlerHook hook = new NiftyRefreshLayout.NiftyUIHandlerHook() {
            @Override
            public void run() {
        ...
        refreshLayout.setRefreshCompleteHook(hook);
    }
    ...
    public void onUIPositionChange(NiftyRefreshLayout frame, boolean isUnderTouch, byte status, NiftyIndicator NiftyIndicator) {
        float percent = Math.min(1f, NiftyIndicator.getCurrentPercent());

        if (status == NiftyRefreshLayout.Nifty_STATUS_PREPARE) {
